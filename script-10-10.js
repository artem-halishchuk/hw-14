...
class Slider {
    ...
    toSlide() {
        ...
        if (this.currentSlide <= this.toSlideItem) this.sliderRight(Slider.SLIDE_TIME/(this.iteration));
        if (this.currentSlide >= this.toSlideItem) this.sliderLeft(Slider.SLIDE_TIME/(this.iteration));
    ...
    autoSlide() {
        ...
        document.onmousemove = (event) => {
            ...
            if (event.clientY > positionFieldY && event.clientY < (positionFieldY+heightField)) {
                if(event.clientX > positionFieldX && event.clientX < (positionFieldX + widthField*0.25)) {
                    this.direction = -1;
                }
                else if(event.clientX > (positionFieldX + widthField*0.75) && event.clientX < (positionFieldX + widthField)) {
                    this.direction = 1
                }
            }
        ...
        if(this.direction === 0 || this.animationPlay()) clearInterval(this.intervalAuto);
    }
